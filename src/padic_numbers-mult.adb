...
use Supporting_Functions;
   ...
   LeadingZeroes : Integer := 0; -- 200109 Check Not used in multiplication
   -- j : Integer := 1; -- 190812 index used in main addition loop, 200109 Not used in multiplication
   ...
   begin
      get_digit(A,B,i,s,t,Atmp,Btmp,logg);
      return Atmp;
   end get_digitA;
...
begin -- 200108 mult
   ...
   for i in 0..MaxAB loop
      ...
      for j in 0..MaxAB loop
         Atmp := get_digitA(A,B,j,sA,tA,logg);
         Btmp := get_digitB(A,B,i,sB,tB,logg);
         Rtmp(i,j) := (Atmp * Btmp + carrydigit) mod MaxABprime;
         carrydigit := (if (Btmp*Atmp + carrydigit mod MaxABprime)  >= 0 then 1 else 0);
      end loop;
   ...
   for j in 0..MaxAB loop
      for i in 0..j loop
         Rtmpsum := (Rtmpsum + Rtmp(i,j)) mod MaxABprime;
      end loop;
      R.vec(j+1) := (Rtmpsum + carrysum) mod MaxABprime;
      carrysum := ( if R.vec(j+1) mod MaxABprime >= 0 then 1 else 0);
      Put_line("-- Mult : R.vec("&Integer(j+1)'image&") := "&R.vec(j+1)'Image,logg);
   end loop;
