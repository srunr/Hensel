...
function add2( A , B : in HenselCode) return HenselCode is
use Supporting_Functions;
   ...
   function MaxDivisor(a : MaxInteger) return MaxInteger
     with Pre => (a /= 0) is
      ...
      i : Integer := 0;
   begin
      -- Put("-- Add2 : MaxDivisor input = "); PutList(s); New_line;
   ...
   function MinDivisor(a : MaxInteger) return MaxInteger
     with Pre => (a /= 0) is
      ...
      i : Integer := 0;
   begin
      -- Put("-- Add2 : MinDivisor input = "); PutList(s); New_line;
      for i in s'Range loop
         r := MIN(r,s(i));
      end loop;
   ...
   begin
      Put_line("-- Add2 : shift_vector_left :  h.vec("&LastPos'image&") = "&MaxInteger'Image(h.vec(LastPos)));
   ...
   begin
      Put_line("-- Add2 : trailing_zeroes : LastIndex = "&LastIndex'Image&" h.size = "&h.size'Image);
      ...
      Put_Line("-- Add2 : trailing_zeroes : found_integer = "&found_integer'Image);
   ...
   begin -- 190514 : Added to catch several carry spillover positions from non-periodic to periodic positions
      while i < MaxInteger(cv'Length-1) and i /= 0 loop
         Put_line("-- Add2 : carryvec("&i'Image&") = "&cv(i)'Image,True);
         exit when cv(i) = 0;
         i := i + 1;
      end loop;
      return i;
   ...
   exception when Constraint_Error =>
         Put_line("-- Add2 : carry_spill_over : Constraint_Error for i = "&i'Image,True);
         return i;
   ...
   begin
      Put_line("-- Add2 : Period_Sum : h.prime = "&h.prime'Image&" FirstIndex = "&FirstIndex'Image&" Period = "&Period'Image&" h.MaxLen = "&h.MaxLen'Image,True);
      for i in FirstIndex..FirstIndex+per-1 loop
         sum := sum + h.vec(i)*h.prime**(i-1);
         Put_Line("-- Add2 : Period_Sum : sum("&i'Image&")= "&sum'Image,True);
      end loop;
      Put_line("-- Add2 : Period_Sum = "&sum'Image,True);
      return sum;
   end Period_Sum;
   ...
   begin
      Put("-- Add2 : ValidateResult H input : ",logg); Put_Hensel_code(H,logg); New_line;
      ratioH := Decode4(H,False);
         H.FirstPerPos := 1;
         H.LastPerPos := H.LastPos;
         H.RecoveryLength := floor(float(H.LastPos)/2.0);
         Put("-- Add2 : ValidateResult recovery attempt input : ",logg); Put_Hensel_code(H,logg); New_line;
         ratioA := Decode4(H,False);
         Put_line("-- Add2 : ValidateResult : recovery ratioH = "&Image(ratioA), logg);
         ratioB := Decode4(EncodeVariableHensel(Numerator(ratioA), Denominator(ratioA), H.prime, H.FirstPerPos, H.LastPerPos),False);
...
begin -- add2
   ...
   Put("-- Addition2 : ",True); Put_Hensel_Code(A,True);Put(" + ",True);Put_Hensel_Code(B,True);Put(" = ",True); New_Line;
   Put_Line("-- Add2 : A.MaxLen = "&A.MaxLen'Image&" B.MaxLen = "&B.MaxLen'Image&" MaxLen = "&MaxLen'Image);
   Put_Line("-- Add2 : MaxAB = "&MaxAB'Image&" ExitIndex = "&ExitIndex'Image&" ABPerLength = "&ABPerLength'Image);
   Put_Line("-- Add2 : A.DenFactor = "&A.DenFactor'Image&" B.DenFactor = "&B.DenFactor'Image);
   Put_Line("-- Add2 : A.NumFactor = "&A.NumFactor'Image&" B.NumFactor = "&B.NumFactor'Image);
   Put_Line("-- Add2 : R.RecoveryLength = "&R.RecoveryLength'Image);
   Put_Line("-- Add2 : APeriod = "&Aperiod'Image&" BPeriod = "&BPeriod'Image&" A.FirstPerPos = "&A.FirstPerPos'Image&" A.LastPerPos = "&A.LastPerPos'Image&" B.FirstPerPos = "&B.FirstPerPos'Image&" B.LastPerPos = "&B.LastPerPos'Image);
   Put_Line("-- Add2 : LCM(APeriod,BPeriod) = "&LCM(MaxInteger(APeriod),MaxInteger(BPeriod))'Image);
   Put("-- Add2 : Decompose(LCM(APeriod,BPeriod)) = ");
   if LCM(MaxInteger(Aperiod),MaxInteger(BPeriod)) /= 0 then
      ...
      Put_Line("-- Add2 : MaxDivisor(LCM(APeriod,BPeriod)) = "&MaxDivisor(LCM(MaxInteger(Aperiod),MaxInteger(BPeriod)))'Image);
      Put_Line("-- Add2 : MinDivisor(LCM(APeriod,BPeriod)) = "&MinDivisor(LCM(MaxInteger(Aperiod),MaxInteger(BPeriod)))'Image);
      Put_Line("-- Add2 : MAX(MaxInteger(A.FirstPerPos),MaxInteger(B.FirstPerPos)) = "&MaxInteger(MAX(MaxInteger(A.FirstPerPos),MaxInteger(B.FirstPerPos)))'Image);
      Put_Line("-- Add2 : Theory R.FirstPerPos <= MAX(MaxInteger(A.FirstPerPos),MaxInteger(B.FirstPerPos))+MinDivisor(LCM(APeriod,BPeriod))+3 = "&
      ...
      Put_Line("-- Add2 : Theory R.LastPerPos <= R.FirstPerPos + ABPerLength - 1 = "&
   ...
   for i in 1..MaxAB loop  -- 190812 Consider to replace with while loop and avoid updating i when leading zeroes.
   ...
   Put_line("-- Add2 (491) : before adj : R.ordinal = "&R.ordinal'Image&" R.FirstPerPos = "&R.FirstPerPos'Image&" R.LastPerPos = "&R.LastPerPos'Image&" R.LastPos = "&R.LastPos'Image);
   Put_line("-- Add2 (492) : before adj : A.ordinal = "&A.ordinal'Image&" A.FirstPerPos = "&A.FirstPerPos'Image&" A.LastPerPos = "&A.LastPerPos'Image&" A.LastPos = "&A.LastPos'Image);
   Put_line("-- Add2 (492) : before adj : B.ordinal = "&B.ordinal'Image&" B.FirstPerPos = "&B.FirstPerPos'Image&" B.LastPerPos = "&B.LastPerPos'Image&" B.LastPos = "&B.LastPos'Image);
   Put_line("-- Add2 (493) : before adj : common_nonperiodic = "&common_nonperiodic'Image);
   ...
   if A.FirstPerPos = 0 and B.FirstPerPos /= 0 then
      Put_Line("-- Add2 : A.FirstPerPos = 0 and B.FirstPerPos /= 0");
      ...
      if LastCarryIndex > 0 then
         Put_Line("-- Add2 : LastCarryIndex > 0");
         ...
         Put_Line("-- Add2 : R.FirstPerPos := B.FirstPerPos + abs(A.ordinal - B.ordinal)");
      ...
      if LastCarryIndex > 0 then
         ...
         Put_Line("-- Add2 : R.FirstPerPos := A.FirstPerPos + abs(A.ordinal - B.ordinal)");
      ...
      if (((APeriod = 1) and (BPeriod /= 1) and (A.vec(A.FirstPerPos) = A.prime-1)
         ...
         Put_Line("-- Add2 : A or B negative integer : Case 4,5,6 and Case 9,13");
         ...
         Put_Line("-- Add2 : R.FirstPerPos = "&R.FirstPerPos'Image);
         ...
         --          end if;
         Put_Line("-- Add2 : 2*Integer(ABPerLength) + R.FirstPerPos = "&Integer(2*Integer(ABPerLength) + R.FirstPerPos)'Image&" < MaxLen = "&MaxLen'Image);
         if (2*Integer(ABPerLength) + R.FirstPerPos < MaxLen) -- 190716 : Not enough hensel digits to compare 2 periods
         then
            Put_Line("-- Add2 : (2*Integer(ABPerLength) + R.FirstPerPos < MaxLen) = "&Boolean(2*Integer(ABPerLength) + R.FirstPerPos < MaxLen)'Image);
         ...
         Put_Line("-- Add2 : (APeriod /= 1 and BPeriod /= 1 and Aperiod = BPeriod and A.FirstPerPos = B.FirstPerPos)");
         ...
         Put_line("-- Add2 : (APeriod /= 1 and BPeriod /= 1) and common_nonperiodic ");
         R.FirstPerPos := Integer(MAX(MaxInteger(A.FirstPerPos),MaxInteger(B.FirstPerPos))) + abs(A.ordinal - B.ordinal);
         Put_line("-- Add2 : carry_spill_over(carryvec,"&R.FirstPerPos'Image&") = "&carry_spill_over(carryvec,R.FirstPerPos)'Image);
         ...
         Put_line("-- Add2 : (APeriod = BPeriod) and not common_nonperiodic");
         R.FirstPerPos := Integer(MAX(MaxInteger(A.FirstPerPos),MaxInteger(B.FirstPerPos))) + abs(A.ordinal - B.ordinal);
         R.LastPerPos := R.FirstPerPos + Integer(ABPerLength - 1);
      elsif not common_nonperiodic then
         Put_Line("-- Add2 : not common_nonperiodic");
         ...
         R.LastPerPos := R.FirstPerPos + Integer(ABPerLength - 1);
         Put_line("-- Add2 : R.FirstPerPos = "&R.FirstPerPos'Image&" R.LastPerPos = "&R.LastPerPos'Image&" R.LastPos = "&R.LastPos'Image);
      end if;
      ...
      if (R.RecoveryLength > R.FirstPerPos + Integer(ABPerLength) - 1) and R.RecoveryLength <= Integer(ExitIndex) then
         Put_Line("-- Add2 : (R.RecoveryLength > R.FirstPerPos + Integer(ABPerLength) - 1) and R.RecoveryLength <= Integer(ExitIndex)");
      ...
      Put_Line("-- Add2 : R.NoPeriod -- Case 3 both A and B are postive integer");
   ...
   Put_line("-- Add2 (634) : R.ordinal = "&R.ordinal'Image&" R.FirstPerPos = "&R.FirstPerPos'Image&" R.LastPerPos = "&R.LastPerPos'Image&" R.LastPos = "&R.LastPos'Image);
   -- R.dot_pos := R.ordinal; -- 190430 Error should not be setting dot_pos to ordinal here !!
   Put_line("-- Add2 : R.dot_pos = "&R.dot_pos'Image&" ExitIndex = "&ExitIndex'Image&" NoPeriod = "&R.NoPeriod'Image&" LastCarryIndex = "&LastCarryIndex'Image&" R.ordinal = "&R.ordinal'Image);
   Put_line("-- Add2 : carry_spill_over(carryvec,"&R.FirstPerPos'Image&") = "&carry_spill_over(carryvec,R.FirstPerPos)'Image);
   Put("-- Add2 : before adjustment of leading/trailing zeroes and period one : ",True); Put_Hensel_code(R,True); New_line;
   ...
   if  trailing_zeroes(z,R,Integer(ExitIndex)) then -- result is a positive integer if trailing zeroes for all ABLastPerPos - ABFirstPerPos
      Put_line("-- Add2 : trailing_zeroes = "&z'Image);
      ...
      Put_Line("-- Add2 : period_one z = "&z'Image);
      R.FirstPerPos := Integer(ExitIndex) - z + 1;
      R.LastPerPos := R.FirstPerPos;
      Put_Line("-- Add2 : period_one adjusted R.FirstPerPos = "&R.FirstPerPos'Image&" R.LastPerPos = "&R.LastPerPos'Image);
      R.NoPeriod := False;
   end if;

   -- 190425 remove leading zeroes and adjust ordinal  !!!
   if leading_zeroes(z, R) then -- normalize hensel vector to a unit.
      Put_line("-- Add2 : leading_zeroes = "&z'Image);
      shift_vector_left(z,R);
      Put("-- Add2 : After adjustment of leading zeroes : ",True); Put_Hensel_code(R,True); New_line;
      ...
      if not R.NoPeriod and A.FirstPerPos /= 0 and B.FirstPerPos /= 0 then -- 190503 Moved out of shift_vector. 190505 Added "and A.FristPerPos /= 0 and B.FirstPerPos /= 0"
         Put_line("-- Add2 : not R.NoPeriod and A.FirstPerPos /= 0 and B.FirstPerPos /= 0");
         if R.LastPerPos = R.FirstPerPos then -- add of rationals A and B results with period 1 and leading zeroes
            Put_line("-- Add2 : R.LastPerPos = R.FirstPerPos ");
            ...
            Put_Line("-- Add2 : A.ordinal = B.ordinal and A.FirstPerPos = B.FirstPerPos and A.LastPerPos = B.LastPerPos and R.LastPerPos + z < R.size");
            R.FirstPerPos := R.FirstPerPos + z;
            R.LastPerPos := R.LastPerPos + z;
            Put_Line("-- Add2 (675): R.ordinal = "&R.ordinal'Image&" R.FirstPerPos = "&R.FirstPerPos'Image&" R.LastPerPos = "&R.LastPerPos'Image&" R.LastPos = "&R.LastPos'Image&" R.size = "&R.size'Image,True);
         elsif A.ordinal = B.ordinal and A.FirstPerPos = B.FirstPerPos and z+1 >= A.FirstPerPos then -- 190613 Test z+1 >= A.FirstPerPos
            Put_Line("-- Add2 : A.ordinal = B.ordinal and A.FirstPerPos = B.FirstPerPos and z+1 >= A.FirstPerPos");
            ...
            R.FirstPerPos := Integer(MAX(MaxInteger(A.FirstPerPos),MaxInteger(B.FirstPerPos))) + abs(A.ordinal - B.ordinal);
            R.LastPerPos := R.FirstPerPos + Integer(ABPerLength) - 1;
            Put_Line("-- Add2 : (681) R.ordinal = "&R.ordinal'Image&" R.FirstPerPos = "&R.FirstPerPos'Image&" R.LastPerPos = "&R.LastPerPos'Image&" R.LastPos = "&R.LastPos'Image&" R.size = "&R.size'Image,True);
         end if;
         ...
         Put_Line("-- Add2 : Case 3 both A and B are postive integer : A.FirstPerPos = 0 and B.FirstPerPos = 0");
         R.ordinal := R.ordinal + z;
         -- R.LastPos := R.LastPerPos; -- 190807 Why this ???
         R.dot_pos := R.ordinal;
      elsif A.FirstPerPos /= 0 or B.FirstPerPos /= 0 then -- 190507 Added case when A or B is a rational, and
         R.ordinal := R.ordinal + z;
         Put_Line("-- Add2 : A.FirstPerPos /= 0 or B.FirstPerPos /= 0");
         if ABPerLength = 1  and R.FirstPerPos - z > 0 then
            Put_Line("-- Add2 : ABPerLength = 1  and R.FirstPerPos - z > 0");
            ...
            Put_Line("-- Add2 : APeriod = BPeriod and A.FirstPerPos = B.FirstPerPos and z > A.FirstPerPos");
            ...
            Put_Line("-- Add2 : APeriod = 1 or BPeriod = 1");
            R.FirstPerPos := R.FirstPerPos - z;
            R.LastPerPos := R.LastPerPos - z;
         end if;
         -- R.LastPos := R.LastPerPos; -- 190807 Why this ???
         ...
         Put_line("-- Add2 : leading_zeroes : Failure : this branch should never happen ???");
         -- R.LastPos := R.LastPerPos; -- 190807 Why this ???
         R.dot_pos := R.ordinal;
      end if;
      Put_Line("-- Add2 : (719) R.ordinal = "&R.ordinal'Image&" R.FirstPerPos = "&R.FirstPerPos'Image&" R.LastPerPos = "&R.LastPerPos'Image&" R.LastPos = "&R.LastPos'Image&" R.size = "&R.size'Image,True);
      ...
      Put_line("-- Add2 : A.FirstPerPos = 1 and B.FirstPerPos = 1 and ExitIndex - 1 - ABPerLength*Floor(float(ExitIndex - 2)/float(ABPerLength)) > 0");
      if ExitIndex - 1 - ABPerLength*floor((float(ExitIndex - 2)/float(ABPerLength))) /= MaxInteger(R.FirstPerPos)  then
         Put_Line("-- Add2 : ExitIndex - 1 - ABPerLength*floor((float(ExitIndex - 2)/float(ABPerLength))) /= MaxInteger(R.FirstPerPos)");
   ...
   Put("-- Add2 : result before validation: ",True); Put_Hensel_code(R,True); Put_Line(" = "&Image(Decode4(R,False)),True);
   ...
   if ValidateResult(R,True) then
       Put_Line("-- Add2 : R.MaxLen = "&R.MaxLen'Image);  -- 190512 : Put_Lines added to check exit values
       Put_Line("-- Add2 : MaxAB = "&MaxAB'Image&" ExitIndex = "&ExitIndex'Image&" ABPerLength = "&ABPerLength'Image);
       Put_Line("-- Add2 : R.DenFactor = "&R.DenFactor'Image);
       Put_Line("-- Add2 : R.NumFactor = "&R.NumFactor'Image);
       Put_line("-- Add2 : R.RecoveryLength = "&R.RecoveryLength'Image);
       Put_line("-- Add2 : R.size = "&R.size'Image&" R.ordinal = "&R.ordinal'Image);
       Put_line("-- Add2 : R.dot_pos = "&R.dot_pos'Image);
       Put("-- Add2 result : ",True); Put_Hensel_code(R,True); Put(" = "&Image(Decode4(R,False)),True);
       return R;
   ...
   -- Put_line("-- Add2 : R.size = "&R.size'Image&" R.ordinal = "&R.ordinal'Image);
   -- Put_line("-- Add2 : R.dot_pos = "&R.dot_pos'Image);
   -- Put("-- Add2 result : ",True); Put_Hensel_code(R,True); New_line;
...
exception
   ...
   when undefined =>
      Put_line("-- Add2 : undefined : Addition undefined for Henselcodes A and B with different primes");
      return R;

   when Constraint_Error =>  -- 190610 Added to check reason for Constraint Error !
      Put_line("-- Add2 : Constraint_Error: R.ordinal = "&R.ordinal'Image&" R.FirstPerPos = "&R.FirstPerPos'Image&" R.LastPerPos = "&R.LastPerPos'Image&" R.LastPos = "&R.LastPos'Image);
      return R;

   when Assertion_Error =>
      Put_Line("-- Add2 : Assertion_Error");
      return R;

   when Addition_Validation_Error =>
      Put_Line("-- Add2 : Addition_Validation_Error");
      return R;
